...
function recommendFertilizer() {
  ...
  if (crop === "rice") plan = "Recommended: Urea + DAP with nitrogen in split doses.";
  if (crop === "wheat") plan = "Recommended: Urea + SSP + MOP with phosphorus as basal.";
  if (crop === "maize") plan = "Recommended: NPK 15:15:15 basal + Urea for top dressing.";
  if (crop === "soybean") plan = "Recommended: SSP + Potash depending on soil.";
  if (crop === "cotton") plan = "Recommended: High nitrogen and potassium strategy.";
...
function quickDemoDisease() {
  ...
  const demoData = {
    ...
    confidence: 0.92,
    treatment: {
      organic: "Neem oil 3% spray, crop rotation, infected leaves remove karein.",
      chemical: "Mancozeb ya Chlorothalonil fungicide recommend dose par spray karein.",
    },
    fertilizerFix: "Potassium (K) level improve karein – plant immunity strong hoti hai.",
    humidity: 78,
    temperature: 29,
  };

  renderDiseaseAdvice(demoData, diseaseBox, adviceBox, riskBox, spreadBox, true);
}
  ...
  try {
    ...
    const data = {
      ...
      treatment: {
        ...
        chemical:
          "Copper oxychloride ya recommended fungicide 10–14 din ke gap par spray karein.",
      },
      fertilizerFix:
        "Balanced NPK follow karein, overhead irrigation kam karein, micronutrients (Zn, B) ka foliar spray karein.",
      humidity: 82,
      temperature: 27,
    };
...
function renderDiseaseAdvice(
  ...
  if (data.humidity > 60 && data.temperature > 24) riskLabel = "High Risk (favourable conditions)";
  ...
  if (riskLabel.startsWith("High")) {
    spread = "⚠ Rapid spread possible next 3–5 days – timely spray & sanitation required.";
...
function showTips() {
  ...
  box.innerHTML = `
    ...
    • Certified, disease-free seeds ka use karein aur seed treatment follow karein.<br />
    • Proper crop rotation karein – ek hi crop baar baar same field mein mat ugayein.<br />
    • Water logging avoid karein, field mein sahi drainage system rakhein.<br />
    • Balanced fertilizer use karein – nitrogen ki over-dose disease ko invite kar sakti hai.<br />
    • Infected leaves/plant ko jaldi remove karein aur field ke bahar destroy karein.<br />
    • Need-based fungicide/pesticide spray karein, hamesha recommended dose & interval follow karein.<br />
  `;
}
